...
async function main() {
  ...
  let pathPrice_address = "0x6C0aC08FCDB0a57302C88AACCD850ccF6A40a681";
  ...
  // let approveTx = await poolsPrice.safeApprove(weth_address, uniswap_routerV2);
  ...
  let amount = ethers.utils.parseEther("6");
  let amount1 = ethers.utils.parseUnits("0.0015", 18);
  let override = {
    value: amount
  }
  ...
  let params = {
    protocolTypes: [3, 3],
    routers: [uniswap_routerV2, uniswap_routerV2],
    paths: [path1, path2],
  ...
  let tx1 = await poolsPrice.requestFlashLoan(
    wmatic_address,
    amount,
    paramsEncodeData
  );
