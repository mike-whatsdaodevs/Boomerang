...
async function main() {
  ...
  let decimal = 8;//await vault.getLinkDecimals(link_matic);
  let tx1 = await vault.setOracle(wmatic_address, link_matic, ethers.utils.parseUnits("1", decimal));
  await tx1.wait();
  console.log(await vault.oracles(wmatic_address));

  let decimal1 = await token.attach(wmatic_address).decimals();
  console.log(decimal);

  let tx = await vault.setTokenDecimals(wmatic_address, ethers.utils.parseUnits("1", decimal1));
  await tx.wait();
  console.log(await vault.tokenDecimals(wmatic_address));
}
